classdef Controller<handle
    ...
    properties
        time_step = [];
        onramp_start_position = []; % 合流車線の開始位置
    ...
    methods
        function obj = Controller(simulation, vehicle_manager)
            ...
            obj.onramp_start_position = simulation.onramp.start_position; % 合流車線の開始位置
        ...
        function mobil(obj, vehicle, politeness_factor)
            ...
            threshold_acceleration = 0.2; % 加速度のしきい値

            if ~isempty(obj.vehicle_manager.find_surround_vehicles_in_lane(vehicle, 'Mainline', 0, 'rear'))
        ...
        function [optimal_u_sequence optimal_tau optimal_gap] = lane_merge_mpc(obj, vehicle, prediction_horizon, tau_interval, weight)
            ...
            jerk_matrix = obj.get_jerk_matrix(prediction_horizon, obj.time_step); % ジャーク行列を取得
            ...
            for gap_idx = 1:2
                ...
                for tau_idx = 1:length(tau_feasible_set)
                    ...
                    % b = [-F_matrix(1:2:end-1, :) * vehicle_state; F_matrix(2*tau_k-1:2:end-1, :) * vehicle_state; - F_matrix(2:2:end, :) * vehicle_state; F_matrix(2:2:end, :) * vehicle_state; vehicle.MAX_JERK * obj.time_step * ones(end_step, 1); -vehicle.MIN_JERK * obj.time_step * ones(end_step, 1)];
                    ...
                    if gap_idx == 1
                        ...
                        b(3*end_step-tau_k+2:4*end_step-tau_k+1) = b(3*end_step-tau_k+2:4*end_step-tau_k+1) - vehicle.MIN_VELOCITY;
                        ...
                        % 後続車両の後に合流する場合の制約条件
                        A = [G_matrix(1:2:end-1, :); G_matrix(2:2:end, :); -G_matrix(2:2:end, :); jerk_matrix; -jerk_matrix];
                        b = [-F_matrix(1:2:end-1, :) * vehicle_state; - F_matrix(2:2:end, :) * vehicle_state; F_matrix(2:2:end, :) * vehicle_state; vehicle.MAX_JERK * obj.time_step * ones(end_step, 1); -vehicle.MIN_JERK * obj.time_step * ones(end_step, 1)];
                        ...
                        b(end_step+1:2*end_step) = b(end_step+1:2*end_step) + vehicle.MAX_VELOCITY;
                        b(2*end_step+1:3*end_step) = b(2*end_step+1:3*end_step) - vehicle.MIN_VELOCITY;
                        b(3*end_step+1) = b(3*end_step+1) + vehicle.input_acceleration;
                        b(4*end_step+1) = b(4*end_step+1) - vehicle.input_acceleration;
            ...
            % 計算時間の測定終了
            time_elapsed = toc(time_start); % 計算時間を取得
            ...
            disp(['Time elapsed for MPC: ', num2str(time_elapsed), ' seconds']); % MPCの計算時間を表示
        ...
        function optimal_u_sequence = cruise_control_mpc(obj, vehicle, prediction_horizon, weight)
            ...
            A = [G_matrix(1:2:end-1, :); -G_matrix(1:2:end-1, :); G_matrix(2:2:end, :); -G_matrix(2:2:end, :); jerk_matrix; -jerk_matrix];
            b = [-F_matrix(1:2:end-1, :) * vehicle_state; F_matrix(1:2:end-1, :) * vehicle_state; -F_matrix(2:2:end, :) * vehicle_state; F_matrix(2:2:end, :) * vehicle_state; vehicle.MAX_JERK * obj.time_step * ones(end_step, 1); -vehicle.MIN_JERK * obj.time_step * ones(end_step, 1)];
            ...
            b(3*end_step+1:4*end_step) = b(3*end_step+1:4*end_step) - vehicle.MIN_VELOCITY;
            b(4*end_step+1) = b(4*end_step+1) + vehicle.acceleration;
            b(5*end_step+1) = b(5*end_step+1) - vehicle.acceleration;
        ...
        function jerk_matrix = get_jerk_matrix(obj, prediction_horizon, time_step)
            % ジャーク行列を計算

            N = prediction_horizon / obj.time_step; % 予測ホライズンを時間ステップで割る
            jerk_matrix = zeros(N, N);

            for i = 1:N
                jerk_matrix(i, i) = 1; % 現在の制御入力
                if i > 1
                    jerk_matrix(i, i-1) = -1; % 前の制御入力
                end
            end
        end
